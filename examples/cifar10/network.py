...
def create_dense_layer(list_of_layers, list_of_nodes, layer_index, node_index, node):
	...
	if len(list_of_nodes) == 0: #this means that this is the first layer after input
		...
		input_flat = tf.reshape(list_of_layers[layer_index], [-1, INPUT_DIM * INPUT_DIM * NUM_CHANNEL])
...
def create_conv_maxpool_layer(list_of_layers, list_of_nodes, layer_index, node_index, node):
	...
	if len(list_of_nodes) == 0: #this means that the current layer is the first layer after input
		...
		#then add a maxpooling layer if it has one
		if node.has_maxpool == "true":
			pool=tf.layers.max_pooling2d(inputs=conv, pool_size=[int(node.poolsize_pool), int(node.poolsize_pool)], data_format="channels_last", strides=int(node.stride_pool), padding="same")
...
def cnn_model_fn(features, labels, mode, params):
	...
	gen=params["gen"]
	input_layer=create_input_layer(features["x"])
